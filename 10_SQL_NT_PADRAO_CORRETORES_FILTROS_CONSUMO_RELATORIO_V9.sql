...
--
-- Corrige:
-- 1) Filtros carregados por tipo, sem misturar tudo numa única resposta.
-- 2) Contagens reais por filtro, sem amostra no navegador.
-- 3) Autoajuste do vínculo do usuário pelo e-mail logado, quando auth_user_id divergir.
-- 4) Relatórios no padrão da Plataforma Corretores.
...
drop policy if exists nt_publicos_preview_anon_v9 on public.nt_talentos_publicos;
drop policy if exists nt_publicos_preview_auth_v9 on public.nt_talentos_publicos;
drop policy if exists nt_filtro_cidade_preview_anon_v9 on public.nt_filtro_cidade;
drop policy if exists nt_filtro_idade_preview_anon_v9 on public.nt_filtro_cidade_idade;
drop policy if exists nt_filtro_cargo_preview_anon_v9 on public.nt_filtro_cidade_cargo;
drop policy if exists nt_filtro_metro_preview_anon_v9 on public.nt_filtro_cidade_metro;

create policy nt_publicos_preview_anon_v9
on public.nt_talentos_publicos
for select
to anon
using (produto_codigo = 'NOVOS_TALENTOS' and ativo = true);

create policy nt_publicos_preview_auth_v9
on public.nt_talentos_publicos
for select
to authenticated
using (produto_codigo = 'NOVOS_TALENTOS' and ativo = true);

create policy nt_filtro_cidade_preview_anon_v9
on public.nt_filtro_cidade
for select
to anon
using (true);

create policy nt_filtro_idade_preview_anon_v9
on public.nt_filtro_cidade_idade
for select
to anon
using (true);

create policy nt_filtro_cargo_preview_anon_v9
on public.nt_filtro_cidade_cargo
for select
to anon
using (true);

create policy nt_filtro_metro_preview_anon_v9
on public.nt_filtro_cidade_metro
for select
to anon
using (true);

create or replace function public.nt_macro_publica_v9(p_regiao text, p_bairro text)
returns text
language sql
stable
as $$
...
create or replace function public.nt_micro_publica_v9(p_micro text, p_estacao text, p_bairro text)
returns text
language sql
stable
as $$
...
create or replace function public.nt_base_publica_v9()
returns table (
  talento_key text,
  nome_mascarado text,
  primeiro_nome text,
...
as $$
  select
    ...
    p.regiao_macro,
    p.micro_regiao,
    public.nt_macro_publica_v9(p.regiao_macro, p.bairro) as macro_calc,
    public.nt_micro_publica_v9(p.micro_regiao, p.estacao_mais_proxima, p.bairro) as micro_calc,
    p.tem_whatsapp,
    p.tem_email,
    p.tem_geo,
...
create or replace function public.nt_opcoes_filtro_publico_v9(
  p_tipo text,
...
as $$
  with base as (
    select *
    from public.nt_base_publica_v9() b
    where
      -- Cidade sempre precisa carregar ampla, igual Corretores.
      ...
      -- Cada filtro ignora ele mesmo para não "travar" as opções.
  ...
  limit case
    when p_tipo = 'cargo' then 600
    when p_tipo = 'bairro' then 600
    when p_tipo = 'micro_regiao' then 600
    else 500
  end;
$$;

create or replace function public.nt_listar_talentos_publico_v9(
  p_cidade text default null,
  p_estado_uf text default null,
  p_regiao_macro text default null,
  p_micro_regiao text default null,
...
begin
  ...
  -- Autoajuste do vínculo, mantendo o padrão da Corretores:
  -- se o e-mail logado bate com a planilha, grava o UID real do Auth.
...
grant execute on function public.nt_macro_publica_v9(text, text) to anon, authenticated;
grant execute on function public.nt_micro_publica_v9(text, text, text) to anon, authenticated;
grant execute on function public.nt_base_publica_v9() to anon, authenticated;
grant execute on function public.nt_opcoes_filtro_publico_v9(text, text, text, text, text, text, text, text, text, text) to anon, authenticated;
grant execute on function public.nt_listar_talentos_publico_v9(text, text, text, text, text, text, text, text, text, integer, integer) to anon, authenticated;
...
group by c.conta_id, c.nome_conta, c.plano_tipo, c.status, c.limite_total, c.usuarios_contratados;
...
-- Diagnóstico esperado: não pode vir só bairro.
select 'cidade' as filtro, count(*) as opcoes from public.nt_opcoes_filtro_publico_v9('cidade', null, null, null, null, null, null, null, null, null)
union all select 'regiao_macro', count(*) from public.nt_opcoes_filtro_publico_v9('regiao_macro', 'SÃO Paulo', 'SP', null, null, null, null, null, null, null)
union all select 'micro_regiao', count(*) from public.nt_opcoes_filtro_publico_v9('micro_regiao', 'SÃO Paulo', 'SP', 'Zona Sul', null, null, null, null, null, null)
union all select 'bairro', count(*) from public.nt_opcoes_filtro_publico_v9('bairro', 'SÃO Paulo', 'SP', 'Zona Sul', null, null, null, null, null, null);
